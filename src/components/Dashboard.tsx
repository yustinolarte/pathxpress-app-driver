...
export function Dashboard({ onNavigate, routeData }: DashboardProps) {
  ...
  const routeId = routeData?.routeId || 'No Route';
  const completedCount = routeData?.deliveries?.filter((d: any) => d.status === 'completed').length || 0;
  ...
  if (!isOnDuty) {
    ...
    isButtonDisabled = true; // Disabled as requested
  }
      ...
      <div className="bg-[#050505] px-6 pt-12 pb-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="w-12 h-12 rounded-full bg-gradient-to-br from-[#e10600] to-[#a00500] flex items-center justify-center text-[#f2f4f8]" style={{ fontFamily: 'Poppins, sans-serif' }}>
              DR
            </div>
          </div>
        </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                ...
                <div className="w-full h-2 bg-[#0a1128] rounded-full overflow-hidden">
                  <div className="h-full bg-gradient-to-r from-[#e10600] to-[#ff3333]" style={{ width: `${progressPercent}%` }}></div>
                </div>
              </div>
            </div>
          </div>
